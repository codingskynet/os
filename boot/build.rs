...
fn main() {
    ...
    println!("cargo:rustc-link-arg-bin=boot=--defsym=KERNEL_LMA_BASE={KERNEL_LMA_BASE:#x}");
    println!("cargo:rustc-link-arg-bin=boot=--defsym=KERNEL_VMA_BASE={KERNEL_VMA_BASE:#x}");
    println!("cargo:rustc-link-arg-bin=boot=--defsym=KERNEL_VMA_OFFSET={KERNEL_VMA_OFFSET:#x}");
    println!("cargo:rustc-link-arg-bin=boot=--defsym=PAGE_SIZE={PAGE_SIZE:#x}");
    println!("cargo:rustc-link-arg-bin=boot=--defsym=HUGE_PAGE_SIZE={HUGE_PAGE_SIZE:#x}");
    println!("cargo:rustc-link-arg-bin=boot=--defsym=STACK_SIZE={STACK_SIZE:#x}");
}
